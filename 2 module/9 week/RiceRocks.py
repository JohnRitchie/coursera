...
# upbeat soundtrack by composer and former IIPP student Emiel Stopler
...
def process_sprite_group(group_sprites, canvas):
    for sprite in group_sprites.copy():
        if sprite.update():
            group_sprites.remove(sprite)
        else:
            sprite.draw(canvas)
            sprite.update()


def group_collide(group_sprites, other_object):
    for sprite in group_sprites.copy():
        if sprite.collide(other_object):
            group_sprites.remove(sprite)
            explosion_group.add(Sprite(sprite.pos, [0, 0], 0, 0, explosion_image,
                                       explosion_info, sound=explosion_sound))
            return True


def group_group_collide(first_group_sprites, second_group_sprites):
    collides = 0

    for sprite in first_group_sprites.copy():
        if group_collide(second_group_sprites, sprite):
            collides += 1
            first_group_sprites.remove(sprite)
...
def stop_game():
    global started, my_ship, rock_group, missile_group
    started = False
