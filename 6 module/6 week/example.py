...
def risk_color(risk):
    ...
    magnitude = float(magnitude)/100
    rgb = colorsys.hls_to_rgb(magnitude, 120.0, -0.7)
...
class PlotCountyData:
    ...
    def load_data(self):
        ...
        return [[tokens[0], float(tokens[1]), float(tokens[2]), int(tokens[3]), float(tokens[4])] for tokens in
                data_tokens]
